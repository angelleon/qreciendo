...
dbConfig.getDB().query(`SELECT * FROM Comentario WHERE activo = TRUE `, callback);
}

let getComentarioById = (idComentario, callback) => {
    dbConfig.getDB().query(`SELECT LIMIT 1 * FROM Comentario WHERE activo = TRUE AND idComentario = ? `, idComentario, callback);
}
